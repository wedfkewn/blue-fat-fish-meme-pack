...
4. 生成透明动态 GIF；
5. 从 `static_sources.json` 下载独立静态作品；
6. 生成动态/静态总览图和 `source_index.json`；
7. 自动提交生成结果。

## 静态表情策略

静态区只收**独立完成的静态梗图/表情图**，不再使用动画抽帧。

当前首批 13 张来自 `lmy414/ai-girl-stickers` 的 DeepSeek 投稿，投稿者为 `MX1951`，其发布清单将这 13 张标记为 **CC0 公共领域**。其他仅标注“授权收录与下载”或授权不明的图片暂不复制到本仓库，只作为后续核对线索。
